 ...
 */
public class GenericConfig extends com.xmlcalabash.core.XProcConfiguration {
    ...
    public static final QName _logger = new QName("", "logger");
    ...
    private static Hashtable<String,Level> validLevels;
    static
        {
            validLevels = new Hashtable<String, Level> ();
            validLevels.put("finest", Level.FINEST);
            validLevels.put("finer", Level.FINER);
            validLevels.put("fine", Level.FINE);
            validLevels.put("info", Level.INFO);
            validLevels.put("warning", Level.WARNING);
            validLevels.put("error", Level.SEVERE);
        }
...
*/
        ...
        for (XdmNode node : new RelevantNodes(doc, Axis.CHILD)) {
            ...
            if (XProcConstants.NS_CALABASH_CONFIG.equals(uri)
                ...
                if ("implementation".equals(localName)) {
                    ...
                    parseInput(node);
                } else if ("log-level".equals(localName)) {
                    parseLogLevel(node);
    ...
    public void setDefaultLogging(String level) {
        if (validLevels.containsKey(level)) {
            defaultLogLevel = validLevels.get(level);
        } else {
            throw new XProcException("Invalid configuration value for default log level: " + level);
        }
    }
    ...
    private void parseLogLevel(XdmNode node) {
        String logger = node.getAttributeValue(_logger);
        String levelStr = node.getAttributeValue(_level);
        Level level;

        if (validLevels.containsKey(levelStr)) {
            level = validLevels.get(levelStr);
        } else {
            throw new XProcException("Logging level is not valid: " + levelStr);
        }

        if (logger == null) {
            defaultLogLevel = level;
        } else {
            logLevel.put(logger,level);
        }
    }
